<?php
  class pegDB
  {
    ...
    /** Connect to a MySQL database to be able to use the methods below.
      */
    function pegDB($base, $server, $user, $pass,
                   $errorConnect="Server connection not possible.",
                   $errorDatabase="Database connection not possible.",
                   $disableErrors=false)
    {
      $this->mtStart    = $this->getMicroTime();
      $this->nbQueries  = 0;
      $this->lastResult = NULL;
      $this->errorsDisabled = $disableErrors;
      $this->connectionRef = mysql_connect($server, $user, $pass) or die($errorConnect);
      mysql_select_db($base) or die($errorDatabase);
    }
    ...
    /** Alias for mysql real escape string
      * @param $string What you want escaped
      * @return The escaped value
      */
    function escape($string) 
    {
      if(isset($this->connectionRef))
        return mysql_real_escape_string((string) $string);
      else
        $this->debugQuery("No DB Connection","Error");
    }
    
    /** Insert an array to the the database, based on keys and values
      * @param $table The table to put the data into
      * @param $array The array of key=>value data
      */
    function insert($table, $array, $debug=false) {
      if(!is_array($array)) {
        $this->debugQuery("Tried to insert non array","Error");
      } else {       
        $keys = "";
        $values = "";
        $size = count($array);
        ...
        foreach(($array) as $key => $value) {
          $keys .= "`".$this->escape($key)."`".($i<$size ? "," : "");
          $values .= "'".$this->escape($value)."'".($i<$size ? "," : "");
          $i++;
        }
        $query = "INSERT INTO `$table` ($keys) VALUES ($values)";
        $this->execute($query, $debug);
      }
    }
  } // class DB
?>
